...
static class ExtendedPlayerControl
{
    ...
    public static bool KnowRoleTarget(PlayerControl seer, PlayerControl target)
    {
        ...
        else if (Options.ImpsCanSeeEachOthersRoles.GetBool() && seer.CheckMMCanSeeImp() && target.CheckMMCanSeeImp() && !Main.PlayerStates[seer.PlayerId].IsNecromancer && !Main.PlayerStates[target.PlayerId].IsNecromancer && !seer.Is(CustomRoles.Narc)) return true;
        ...
        else if (Madmate.ImpKnowWhosMadmate.GetBool() && target.Is(CustomRoles.Madmate) && seer.CheckMMCanSeeImp() && !Main.PlayerStates[seer.PlayerId].IsNecromancer && !Main.PlayerStates[target.PlayerId].IsNecromancer && !seer.Is(CustomRoles.Narc)) return true;
        else if (seer.CheckMMCanSeeImp() && target.GetCustomRole().IsGhostRole() && target.GetCustomRole().IsImpostor() && !Main.PlayerStates[seer.PlayerId].IsNecromancer && !Main.PlayerStates[target.PlayerId].IsNecromancer && !seer.Is(CustomRoles.Narc)) return true;
    ...
    public static bool ShowSubRoleTarget(this PlayerControl seer, PlayerControl target, CustomRoles subRole = CustomRoles.NotAssigned)
    {
        ...
        else if (Options.ImpsCanSeeEachOthersAddOns.GetBool() && seer.CheckMMCanSeeImp() && !seer.Is(CustomRoles.Narc) && target.CheckMMCanSeeImp() && !subRole.IsBetrayalAddon() && subRole is not CustomRoles.Torch) return true;
    ...
    public static bool KnowSubRoleTarget(PlayerControl seer, PlayerControl target)
    {
        ...
        if (seer.CheckMMCanSeeImp( ) && !seer.Is(CustomRoles.Narc))
        {
            // Imp know Madmate
