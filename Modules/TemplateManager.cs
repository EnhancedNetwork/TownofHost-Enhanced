...
public static class TemplateManager
{
    ...
    public static void SendTemplate(string str = "", byte playerId = 0xff, bool noErr = false)
    {
        ...
        if (sendList.Count == 0 && !noErr)
        {
            ...
            else Utils.SendMessage(string.Format(GetString("Message.TemplateNotFoundClient"), str), playerId, addtoHistory: false);
        }
        else foreach (string x in sendList.ToArray())
            {
                ...
                Utils.SendMessage(ApplyReplaceDictionary(rmv), playerId, title, addtoHistory: false);
            }
    }
