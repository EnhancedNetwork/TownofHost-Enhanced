...
public static class NameColorManager
{
    ...
    private static bool KnowTargetRoleColor(PlayerControl seer, PlayerControl target, bool isMeeting, out string color)
    {
        ...
        // Impostor & Madmate
        if (seer.Is(CustomRoleTypes.Impostor) && target.Is(CustomRoleTypes.Impostor)) color = (target.Is(CustomRoles.Egoist) && Egoist.ImpEgoistVisibalToAllies.GetBool() && seer != target) ? Main.roleColors[CustomRoles.Egoist] : Main.roleColors[CustomRoles.Impostor];
        ...
        if (seer.Is(CustomRoleTypes.Impostor) && target.Is(CustomRoles.Minion)) color = Main.roleColors[CustomRoles.Madmate];
