...
public static class NameColorManager
{
    ...
    private static bool KnowTargetRoleColor(PlayerControl seer, PlayerControl target, bool isMeeting, out string color)
    {
        ...
        if (seer.CheckImpTeamCanSeeTeammates() && target.GetCustomRole().IsGhostRole() && target.GetCustomRole().IsImpostor() && !Main.PlayerStates[seer.PlayerId].IsNecromancer && !Main.PlayerStates[target.PlayerId].IsNecromancer) color = Main.roleColors[CustomRoles.Madmate];
        ...
        // Narc
        if (seer.Is(CustomRoles.Narc) && seer.CheckImpTeamCanSeeTeammates() && target.Is(CustomRoles.Madmate) && Madmate.ImpKnowWhosMadmate.GetBool() && Narc.NarcCanSeeTeammates.GetBool() && !Main.PlayerStates[seer.PlayerId].IsNecromancer && !Main.PlayerStates[target.PlayerId].IsNecromancer) color = Main.roleColors[CustomRoles.Traitor];
        ...
        if (color != "" && color != string.Empty) return true;
            ...
            || (seer.Is(CustomRoles.Madmate) && target.CheckImpTeamCanSeeTeammates() && Madmate.MadmateKnowWhosImp.GetBool() && !Main.PlayerStates[seer.PlayerId].IsNecromancer && !Main.PlayerStates[target.PlayerId].IsNecromancer && !seer.Is(CustomRoles.Narc))
            || (seer.CheckImpTeamCanSeeTeammates() && !seer.Is(CustomRoles.Narc) && target.Is(CustomRoles.Madmate) && Madmate.ImpKnowWhosMadmate.GetBool() && !Main.PlayerStates[seer.PlayerId].IsNecromancer && !Main.PlayerStates[target.PlayerId].IsNecromancer)
