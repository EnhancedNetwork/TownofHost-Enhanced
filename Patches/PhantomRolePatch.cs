...
public static class PhantomRolePatch
{
    ...
    */
    ...
    private static void CheckVanish_Prefix(PlayerControl __instance)
    {
        ...
        {
            if (!target.IsAlive() || phantom == target || target.AmOwner || !(target.HasDesyncRole() || Main.PlayerStates[target.PlayerId].IsNecromancer || Main.PlayerStates[target.PlayerId].IsRandomizer || phantom.HasDesyncRole())) continue;
