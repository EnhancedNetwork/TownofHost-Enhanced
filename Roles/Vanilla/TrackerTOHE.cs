...
internal class TrackerTOHE : RoleBase
{
    ...
    public override void SetupCustomOption()
    {
        ...
        TrackCooldown = IntegerOptionItem.Create(Id + 2, GeneralOption.TrackerBased_TrackingCooldown, new(5, 120, 5), 15, TabGroup.CrewmateRoles, false)
            .SetParent(Options.CustomRoleSpawnChances[CustomRoles.TrackerTOHE])
            .SetValueFormat(OptionFormat.Seconds);
        TrackDuration = IntegerOptionItem.Create(Id + 3, GeneralOption.TrackerBased_TrackingDuration, new(0, 10, 1), 1, TabGroup.CrewmateRoles, false)
            .SetParent(Options.CustomRoleSpawnChances[CustomRoles.TrackerTOHE])
            .SetValueFormat(OptionFormat.Seconds);
        TrackDelay = IntegerOptionItem.Create(Id + 4, GeneralOption.TrackerBased_TrackingDelay, new(5, 120, 5), 30, TabGroup.CrewmateRoles, false)
            .SetParent(Options.CustomRoleSpawnChances[CustomRoles.TrackerTOHE])
            .SetValueFormat(OptionFormat.Seconds);
    }
