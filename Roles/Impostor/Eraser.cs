...
internal class Eraser : RoleBase
{
    ...
    public override bool CheckVote(PlayerControl player, PlayerControl target)
    {
        ...
        if (targetRole.IsTasklessCrewmate() || targetRole.IsNeutral() || target.Is(CustomRoles.Rebel) || Main.TasklessCrewmate.Contains(target.PlayerId) || CopyCat.playerIdList.Contains(target.PlayerId) || target.Is(CustomRoles.Stubborn))
        {
