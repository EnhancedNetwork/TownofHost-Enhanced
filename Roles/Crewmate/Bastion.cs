...
internal class Bastion : RoleBase
{
    ...
    public override bool OnCoEnterVentOthers(PlayerPhysics physics, int ventId)
    {
        ...
        if (pc.Is(Custom_Team.Crewmate) && !pc.Is(CustomRoles.Bastion) && (DoubleAgent.DoubleAgentCanDiffuseBombs.GetBool() && !pc.Is(CustomRoles.DoubleAgent)) && !pc.IsCrewVenter() && !CopyCat.playerIdList.Contains(pc.PlayerId) && !Main.TasklessCrewmate.Contains(pc.PlayerId)) 
        {
