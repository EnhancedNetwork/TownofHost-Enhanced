...
internal class Grenadier : RoleBase
{
    ...
    public static void ApplyGameOptionsForOthers(IGameOptions opt, PlayerControl player)
    {
        ...
        if ((GrenadierBlinding.Any() &&
            (player.GetCustomRole().IsImpostorV4Narc() ||
            (player.GetCustomRole().IsNeutralV4Narc() && GrenadierCanAffectNeutral.GetBool()))
            )
    ...
    public override void OnEnterVent(PlayerControl pc, Vent vent)
    {
        if (AbilityLimit >= 1)
        {
            ...
            else
            {
                ...
                Main.AllPlayerControls.Where(x => x.IsModded())
                    .Where(x => x.GetCustomRole().IsImpostorV4Narc() || (x.GetCustomRole().IsNeutralV4Narc() && GrenadierCanAffectNeutral.GetBool()))
                    .Do(x => x.RPCPlayCustomSound("FlashBang"));
            }
