...
internal class Reverie : RoleBase
{
    ...
    public override bool OnCheckMurderAsKiller(PlayerControl killer, PlayerControl target)
    {
        ...
        float kcd;
        if ((!target.GetCustomRole().IsCrewmate() && target.Is(CustomRoles.Rebel) && !target.Is(CustomRoles.Trickster)) || (ConvertedReverieRogue.GetBool() && killer.GetCustomSubRoles().Any(subrole => (subrole.IsConverted() && subrole != CustomRoles.Soulless) || subrole == CustomRoles.Rebel))) // if killed non crew or if converted
