...
internal class Marshall : RoleBase
{
    ...
    private static bool IsMarshallTarget(PlayerControl seer) => CustomRoles.Marshall.HasEnabled() && ((seer.Is(Custom_Team.Crewmate) || seer.Is(CustomRoles.Narc)) && !seer.Is(CustomRoles.Rebel));
    ...
    private static bool VisibleToCrewmate(PlayerControl seer, PlayerControl target) => target.GetPlayerTaskState().IsTaskFinished && target.Is(CustomRoles.Marshall) && ((seer.Is(Custom_Team.Crewmate) || seer.Is(CustomRoles.Narc)) && !seer.Is(CustomRoles.Rebel));
