...
internal class Marshall : RoleBase
{
    ...
    private static bool IsMarshallTarget(PlayerControl seer) => CustomRoles.Marshall.HasEnabled() && seer.IsNonRebelCrewmate();
    ...
    private static bool VisibleToCrewmate(PlayerControl seer, PlayerControl target) => target.GetPlayerTaskState().IsTaskFinished && target.Is(CustomRoles.Marshall) && seer.IsNonRebelCrewmate();
