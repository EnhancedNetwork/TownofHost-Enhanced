...
internal class FortuneTeller : RoleBase
{
    ...
    public override bool HideVote(PlayerVoteArea pva) => HidesVote.GetBool() && TempCheckLimit > 0 && didVote.Contains(pva.TargetPlayerId);
    ...
    public override bool CheckVote(PlayerControl player, PlayerControl target)
    {
        ...
        SendMessage(ColorString(GetRoleColor(CustomRoles.FortuneTeller), string.Format(GetString("VoteAbilityUsed"), GetString("FortuneTeller"))), player.PlayerId, title: GetString("VoteHasReturned"));
        return false;
    }
