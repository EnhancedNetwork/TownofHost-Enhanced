...
internal class Medium : RoleBase
{
    ...
    public override void OnOthersMeetingHudStart(PlayerControl pc)
    {
        ...
        if (ContactPlayer.ContainsValue(pc.PlayerId))
            AddMsg(string.Format(GetString("MediumNotifySelf"), Main.AllPlayerNames[ContactPlayer.FirstOrDefault(x => x.Value == pc.PlayerId).Key], _Player.GetAbilityUseLimit()), pc.PlayerId, ColorString(GetRoleColor(CustomRoles.Medium), GetString("MediumTitle")));
