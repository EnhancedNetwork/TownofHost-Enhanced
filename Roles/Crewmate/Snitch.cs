...
internal class Snitch : RoleBase
{
    ...
    private static bool IsSnitchTarget(PlayerControl target)
        => HasEnabled && (target.GetCustomRole().IsImpostorTeamV3() && !target.Is(CustomRoles.Trickster) || (target.IsNeutralKiller() && CanFindNeutralKiller) || (target.IsNeutralApocalypse() && CanFindNeutralApocalypse) || (target.IsPlayerCoven() && CanFindCoven) || (target.Is(CustomRoles.Madmate) && CanFindMadmate) || (target.Is(CustomRoles.Rascal) && CanFindMadmate));
