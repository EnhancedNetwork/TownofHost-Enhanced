...
namespace TOHE.Roles.Neutral
{
    internal class Arsonist : RoleBase
    {
        ...
        public override bool CanUseKillButton(PlayerControl pc)
            => Arsonist.ArsonistCanIgniteAnytime.GetBool() ? Utils.GetDousedPlayerCount(pc.PlayerId).Item1 < ArsonistMaxPlayersToIgnite.GetInt() : !pc.IsDouseDone();
        public override bool CanUseImpostorVentButton(PlayerControl pc)
            => pc.IsDouseDone() || (ArsonistCanIgniteAnytime.GetBool() && (Utils.GetDousedPlayerCount(pc.PlayerId).Item1 >= ArsonistMinPlayersToIgnite.GetInt() || pc.inVent));
        ...
        public override string GetProgressText(byte playerId, bool comms)
        {
            ...
            if (!Arsonist.ArsonistCanIgniteAnytime.GetBool()) return ColorString(GetRoleColor(CustomRoles.Arsonist).ShadeColor(0.25f), $"({doused.Item1}/{doused.Item2})");
            else return ColorString(GetRoleColor(CustomRoles.Arsonist).ShadeColor(0.25f), $"({doused.Item1}/{Arsonist.ArsonistMaxPlayersToIgnite.GetInt()})");
        }
        ...
        public override void OnCoEnterVent(PlayerPhysics __instance, int ventId)
        {
            ...
            if (AmongUsClient.Instance.IsGameStarted)
            {
                ...
                else if (Arsonist.ArsonistCanIgniteAnytime.GetBool())
                {
                    var douseCount = Utils.GetDousedPlayerCount(__instance.myPlayer.PlayerId).Item1;
                    if (douseCount >= Arsonist.ArsonistMinPlayersToIgnite.GetInt()) // Don't check for max, since the player would not be able to ignite at all if they somehow get more players doused than the max
                    {
                        if (douseCount > Arsonist.ArsonistMaxPlayersToIgnite.GetInt()) Logger.Warn("Arsonist Ignited with more players doused than the maximum amount in the settings", "Arsonist Ignite");
                        foreach (var pc in Main.AllAlivePlayerControls)
                        {
