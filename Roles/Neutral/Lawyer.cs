    ...
    public override void Add(byte playerId)
    {
        ...
        if (AmongUsClient.Instance.AmHost && lawyer.IsAlive())
        {
            ...
            foreach (var target in Main.AllPlayerControls)
            {
                ...
                else if (!CanTargetCrewmate.GetBool() && target.IsNonRebelCrewmate()) continue;
                else if (!CanTargetJester.GetBool() && target.Is(CustomRoles.Jester)) continue;
                else if (target.IsRebelNeutralV3() && !target.IsNeutralKiller() && !target.Is(CustomRoles.Jester) && !target.IsNeutralApocalypse()) continue;
